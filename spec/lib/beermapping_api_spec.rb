...
describe "BeermappingApi" do
  ...
  it "When HTTP GET returns no entries, return empty list" do
  ...
  it "When HTTP GET returns multiple entries, they are parsed and returned" do
    ...
    place2 = places[4]
    expect(place2.name).to eq("Kaisla")
    expect(place2.street).to eq("Vilhonkatu 4")
    place3 = places[11]
    expect(place3.name).to eq("Brewdog Helsinki")
    expect(place3.street).to eq("Tarkk'ampujankatu 20")
  end
